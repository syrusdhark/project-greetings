-- First, update existing super_admin and admin users to passholder role
UPDATE public.user_profiles 
SET role = 'school_partner'::user_role 
WHERE role IN ('super_admin'::user_role, 'admin'::user_role);

-- Drop the existing enum and recreate with new values
DROP TYPE IF EXISTS user_role CASCADE;
CREATE TYPE user_role AS ENUM ('passholder', 'school_partner', 'regular_user');

-- Recreate the user_profiles table role column with new enum
ALTER TABLE public.user_profiles 
ALTER COLUMN role TYPE user_role USING 'school_partner'::user_role;
...
-- Update RLS policies to use passholder instead of super_admin/admin
...
DROP POLICY IF EXISTS "Admins can view their own profile" ON user_profiles;
CREATE POLICY "Passholders can view their own profile" 
ON user_profiles FOR SELECT 
TO authenticated 
...
-- Update bookings policy to include passholder
...
-- Set testadmins@gmail.com to passholder role
UPDATE public.user_profiles 
